...
def _eval(coco_gt, image_ids, pred_json_path):
    ...
    recall = coco_eval.stats[8]     # Recall@100 IoU=0.5:0.95 (area=all)
    precision = coco_eval.stats[0]  # Same as mAP@[0.5:0.95] (proxy for overall precision)

    # Print metrics
    print(f"mAP@0.5:0.95 (mAP50-95): {mAP_50_95:.4f}")
    print(f"mAP@0.5 (mAP50): {mAP_50:.4f}")
    print(f"Recall: {recall:.4f}")
    print(f"Precision (proxy): {precision:.4f}")
